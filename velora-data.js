// ============================================================
//  VELORA — velora-data.js  (DÜZELTİLMİŞ)
//  Admin paneli ile siteyi birbirine bağlayan veri katmanı.
//  index.html, category.html, product.html, wishlist.html
//  sayfalarında script.js'den ÖNCE yüklenmelidir.
// ============================================================
...
(function () {
  ...
  //  velora_products localStorage key'inden okur.
  // ----------------------------------------------------------
  ...
  window.vValidateCoupon = function (code) {
    ...
    try {
      ...
      for (var i = 0; i < coupons.length; i++) {
        if (coupons[i].code === normalised) { coupon = coupons[i]; break; }
      }
  ...
  // ----------------------------------------------------------
  //  localStorage'da ilk kez varsayılan ürünleri yükle
  //  (admin paneli açılmadan önce site çalışsın diye)
  // ----------------------------------------------------------
  (function seedDefaults() {
    try {
      var existing = localStorage.getItem("velora_products");
      if (!existing) {
        localStorage.setItem("velora_products", JSON.stringify(VELORA_DEFAULT_PRODUCTS));
      }
      var existingCoupons = localStorage.getItem("velora_coupons");
      if (!existingCoupons) {
