// =====================
// ÜRÜNLER — Admin panelinden gelir (velora-data.js gereklidir)
// =====================

// velora-data.js önce yüklenmiş olmalı (index.html'de script.js'den önce)
// vGetProducts() → localStorage'daki velora_products'ı okur
// Admin panelinden eklenen tüm ürünler otomatik buraya gelir.
const products = (typeof vGetProducts === "function") ? vGetProducts() : [];
...
// KUPON SİSTEMİ
// =====================
// ✅ Artık admin panelinden eklenen kuponlar velora-data.js üzerinden gelir.
// vValidateCoupon() localStorage'daki velora_coupons'ı okur.
...
function applyCoupon() {
  ...
  // ✅ Admin panelindeki kuponları doğrula
  const result = (typeof vValidateCoupon === "function")
    ? vValidateCoupon(kod)
    : { valid: false, discount: 0, message: "Geçersiz kupon kodu." };
  ...
  aktifKupon = { kod: kod, yuzde: result.discount };
  ...
  if (appliedCodeText) appliedCodeText.textContent = `${kod} (%${result.discount} indirim)`;
